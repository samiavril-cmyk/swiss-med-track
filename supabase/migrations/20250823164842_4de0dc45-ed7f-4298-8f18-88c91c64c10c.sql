-- Extend the procedures system for FMH tracking with modules
...
-- Add role_in_surgery enum if not exists
DO $$ BEGIN
    CREATE TYPE procedure_role AS ENUM ('responsible', 'instructing', 'assistant');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

-- Update procedure_logs to use enum for role_in_surgery
ALTER TABLE public.procedure_logs 
ALTER COLUMN role_in_surgery TYPE procedure_role USING role_in_surgery::procedure_role;
...
CREATE OR REPLACE FUNCTION public.calculate_weighted_score(role_type procedure_role)
RETURNS decimal AS $$
BEGIN
  RETURN (
    SELECT weight_factor 
    FROM public.procedure_role_weights 
    WHERE role_type::text = role_type::text
    LIMIT 1
  );
END;
