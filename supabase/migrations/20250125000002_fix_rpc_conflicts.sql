-- Fix RPC function conflicts - Remove all conflicting versions and create one definitive version
-- This migration resolves the endless loading issue caused by multiple conflicting RPC functions

-- Drop all existing versions of get_module_progress function
DROP FUNCTION IF EXISTS public.get_module_progress(UUID, TEXT);
DROP FUNCTION IF EXISTS public.get_module_progress(uuid, text);

-- Create the definitive, working version of get_module_progress function
...
-- Ensure procedure_logs table allows all role types
...
ALTER TABLE public.procedure_logs ADD CONSTRAINT procedure_logs_role_in_surgery_check 
  CHECK (role_in_surgery IN ('primary', 'responsible', 'instructing', 'assistant', 'assist', 'observer'));
