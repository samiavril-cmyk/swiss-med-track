...
interface VerticalCourseTimelineProps {
  courses: Course[];
  year: number;
}

export const VerticalCourseTimeline: React.FC<VerticalCourseTimelineProps> = ({ courses, year }) => {
    ...
    <Card className="medical-card">
      ...
      <CardContent>
        <div className="relative">
          {/* Vertical Timeline line - like in the image */}
          ...
          <div className="space-y-8">
            {sortedCourses.map((course, index) => (
              <div key={course.id} className="relative flex items-start gap-6">
                {/* Timeline node - like in the image */}
                <div className="relative z-10 flex-shrink-0">
                  <div className={`w-4 h-4 rounded-full border-2 border-white shadow-lg ${getStatusColor(course.status)}`}>
                    {/* Type indicator */}
                    <div className={`absolute -top-1 -right-1 w-2 h-2 rounded-full ${getTypeColor(course.type)}`}></div>
                  </div>
                </div>

                {/* Course content - alternating left/right like in the image */}
                <div className={`flex-1 min-w-0 ${index % 2 === 0 ? 'ml-0' : 'mr-0'}`}>
